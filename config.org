...
(use-package corfu
  ...
  (global-corfu-mode)
  :custom
  (corfu-auto t)                 ;; إكمال تلقائي فور الكتابة
  (corfu-auto-delay 0.1)        ;; سرعة الاستجابة
  (corfu-auto-prefix 1)         ;; يبدأ بعد أول حرف
  (corfu-quit-at-boundary nil)  ;; مهم جداً لإكمال مسارات الملفات (لا يغلق عند السلاش)
  (corfu-preview-current nil)   ;; تحسين الأداء في ويندوز
  :config
  ;; جعل مفتاح Tab يقوم بالإكمال إذا كان هناك اقتراح
  (setq tab-always-indent 'complete)

  ;; ربط مفاتيح التنقل داخل القائمة المنبثقة
  (keymap-set corfu-map "TAB" #'corfu-next)
  (keymap-set corfu-map "<tab>" #'corfu-next)
  (keymap-set corfu-map "S-TAB" #'corfu-previous)
  (keymap-set corfu-map "<backtab>" #'corfu-previous)
  
  ;; اختياري: جعل Enter لا يختار من القائمة إلا إذا ضغطت يدوياً (لتجنب الاختيارات الخاطئة)
  ;; (setq corfu-return-complete nil)
  )

(use-package cape
  :ensure t
  :init
  ;; وظيفة إعداد المصادر (Capfs) بترتيب ذكي
  (defun osama-setup-eglot-capfs ()
    (setq-local completion-at-point-functions
                (list (cape-capf-super
                       #'eglot-completion-at-point ;; السيرفر أولاً
                       #'emmet-completion-at-point  ;; ثم Emmet (للأكواد المختصرة)
                       #'cape-dabbrev)             ;; ثم الكلمات الموجودة في المستند
                      #'cape-file                   ;; الملفات بشكل منفصل لضمان عملها عند السلاش
                      #'cape-keyword)))             ;; الكلمات المفتاحية للغة

  ;; تفعيل الإعداد فور تشغيل Eglot
  (add-hook 'eglot-managed-mode-hook #'osama-setup-eglot-capfs)

  ;; إضافة مصادر عامة لكل الأنماط (مثل إكمال الملفات في أي مكان)
  (add-to-list 'completion-at-point-functions #'cape-file)
  (add-to-list 'completion-at-point-functions #'cape-dabbrev))

;; إعدادات إضافية لضمان البحث عن الكلاسات في كل الملفات المفتوحة
...
(use-package eglot
  ...
  (add-to-list 'eglot-server-programs
               `((js-mode rjsx-mode typescript-mode) . ("typescript-language-server" "--stdio"))))
#+end_src
...
(use-package vterm
  :ensure t
  :config
  ;; تأكد من مسار bash الخاص بـ MSYS2
  (setq vterm-shell "C:/msys64/mingw64/bin/bash.exe"))
#+end_src
...
(use-package emmet-mode
  :ensure t
  :hook ((html-mode css-mode rjsx-mode typescript-mode) . emmet-mode)
  :bind
...
*** The =osama-emacs-dev.el= section for ~cpp~ compiler

#+begin_src emacs-lisp :tangle "~/.emacs.d/modules/osama-emacs-dev.el" :mkdirp yes
(defun osama-cpp-compile ()
  "Compile and run current C++ file, handling spaces in path."
  (interactive)
  (let* ((src (shell-quote-argument buffer-file-name))
         (out (shell-quote-argument (file-name-sans-extension buffer-file-name))))
    (compile (format "g++ -Wall %s -o %s && %s" src out out))))
#+end_src
