...
(use-package cape
  :ensure t
  :bind
  (("C-c f" . cape-file)      ;; إكمال الملفات
   ("C-c d" . cape-dabbrev)  ;; كلمات من البفرات
   ("C-c k" . cape-keyword)  ;; كلمات اللغة
   ("C-c s" . cape-symbol))  ;; symbols
  :init
...
#+begin_src emacs-lisp :tangle "~/.emacs.d/modules/osama-emacs-dev.el" :mkdirp yes
;; Web Mode - للملفات المختلطة (HTML, JSX, TSX)
(use-package web-mode
  ...
  :config
  (setq web-mode-markup-indent-offset 2) ; إزاحة HTML
  (setq web-mode-code-indent-offset 2)   ; إزاحة JS/TS
  (setq web-mode-css-indent-offset 2))   ; إزاحة CSS داخل الملف

;; TypeScript Mode - للملفات البرمجية فقط (بدون UI)
(use-package typescript-mode
  :mode "\\.ts\\'"
  :hook (typescript-mode . eglot-ensure))

;; CSS Mode - للملفات المنفصلة
