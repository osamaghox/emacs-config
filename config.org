...
* TODO New Edits
- [ ] edit yasnippt (https://www.youtube.com/watch?v=W-bRZlseNm0) and make for example (<cc) to write compiler command in eshell
...
(defun my-eglot-mode-hook ()
  ;; Combine eglot completion with other sources (like dabbrev-code)
  ;; `cape-super-capf` allows mixing and matching different sources.
  (add-hook 'completion-at-point-functions
            (lambda ()
              (interactive)
              (cape-super-capf
               ;; `eglot-completion-at-point` is Eglot's main completion function
               'eglot-completion-at-point
               ;; `cape-dabbrev-code` for context-aware dabbrev completion
               'cape-dabbrev-code
               ;; `cape-yasnippet` if you use yasnippet
               ;; 'cape-yasnippet
               )))

  ;; Optional but recommended: Add `cape-capf-buster` to the eglot capf
  ;; This helps prevent caching issues with some language servers.
  ;; It ensures candidates are re-requested if the input changes.
  (advice-add 'eglot-completion-at-point :around #'cape-capf-buster))

;; Add the hook to eglot's major modes
(add-hook 'eglot-hook 'my-eglot-mode-hook))
...
(use-package web-mode
  :ensure t
  :mode ("\\.html?\\'" "\\.css\\'" "\\.js\\'" "\\.phtml\\'" "\\.ts\\'") ; Define file extensions for web-mode
  :config
  ;; Customize indentation (optional)
...
#+begin_src emacs-lisp :tangle "~/.emacs.d/modules/osama-emacs-dev.el" :mkdirp yes
;; Emmet Mode - يعمل مع الجميع
(use-package emmet-mode
  :ensure t
  :hook (web-mode css-mode typescript-mode)
  :config
...
#+begin_src emacs-lisp :tangle "~/.emacs.d/modules/osama-emacs-dev.el" :mkdirp yes
(defun osama-cpp-run-eshell ()
  (interactive)
  (unless buffer-file-name
    (error "الملف غير محفوظ"))
  (let* ((buf (get-buffer-create "*eshell-cpp*"))
