...
*** The =osama-emacs-dev.el= section for ~c++-mode~

#+begin_src emacs-lisp :tangle "~/.emacs.d/modules/osama-emacs-dev.el" :mkdirp yes
(use-package c++-mode
  :ensure nil   ;; built-in
  :bind (:map c++-mode-map
          ("C-c m v" . osama-c++-run-and-compile))
  :config
  ;; C++ Compiler G++
  (defun osama-c++-run-and-compile ()
    "Compile current C++ file, open eshell on the right, and a temporary compilation window at the bottom."
    (interactive)
    (let* ((src  (buffer-file-name))
      ...
      ;; 1. حفظ الملف تلقائياً لتجنب رسائل التأكيد
      (save-buffer)

      ;; 2. تنظيف الشاشة وتقسيمها لليمين من أجل Eshell
      (delete-other-windows)
      (split-window-right)

      ;; 3. إعداد مكان ظهور نافذة الـ Compilation (في الأسفل وبحجم صغير)
      ;; هذا الجزء يضمن أن النافذة ستحذف عند ضغط 'q'
      (setq display-buffer-alist
            '(("\\*compilation\\*"
               (display-buffer-reuse-window display-buffer-at-bottom)
               (window-height . 10))))

      ;; 4. البدء بعملية الكومبايل
      (compile cmd)

      ;; 5. الانتقال للنافذة اليمنى لفتح Eshell
      (other-window 1)
      (let ((eshell-buffer-name "*cpp-eshell*"))
        (eshell)
        (goto-char (point-max))
        ;; مسح السطر الحالي إذا كان فيه نص قديم وكتابة الأمر الجديد
