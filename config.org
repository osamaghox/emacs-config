...
(use-package cc-mode
  ...
  (defun osama-c++-run-and-compile ()
    "Compile C++, clean eshell on right, and prepare run command."
    (interactive)
    (let* ((src  (buffer-file-name))
      ...
      ;; 3. إعداد نافذة الكومبايل بالأسفل (تختفي بـ q)
      ...
      ;; 4. تنظيف الـ Eshell وتجهيز الأمر
      (other-window 1)
      (let ((eshell-buffer-name "*cpp-eshell*"))
        (eshell)
        ;; إنهاء أي عملية سابقة ومسح البافر
        (let ((proc (get-buffer-process (current-buffer))))
          (when proc (kill-process proc)))
        (let ((inhibit-read-only t))
          (erase-buffer))
        
        ;; إعادة تشغيل المود لضمان الـ prompt والكتابة فوقه
        (eshell-mode) 
        (goto-char (point-max))
        (insert
