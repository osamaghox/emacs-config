...
(use-package cape
  ...
  :init
  ;; Configure global defaults outside of Eglot managed buffers
  (setq-default completion-at-point-functions
                (append (default-value 'completion-at-point-functions)
                        (list #'cape-dabbrev #'cape-file #'cape-abbrev)))

  ;; Make cape-dabbrev search all buffers
  (setq cape-dabbrev-buffer-function #'buffer-list)

  ;; Define the function inline to handle Eglot's specific local configuration
...
;; Smart function to filter classes and IDs between HTML and CSS
  (defun my-css-smart-dabbrev-capf ()
    "دالة إكمال مخصصة لـ CSS تفرق بين الـ Class والـ ID."
    (when (derived-mode-p 'css-mode 'scss-mode)
      (let* ((end (point))
             (beg (save-excursion (skip-chars-backward "A-Za-z0-9_-") (point)))
             (char-before (char-before beg)))
        ;; التحقق من السياق: هل نحن بعد . أو #
        (when (member char-before '(?\. ?#))
          (let ((result (cape-dabbrev-keywords)))
            (list beg end result
                  :exclusive 'no
                  :company-kind (lambda (_) (if (eq char-before ?#) 'variable 'value))
                  :annotation-function (lambda (_) 
                                         (if (eq char-before ?#) " [ID]" " [Class]")))))))))
#+end_src
