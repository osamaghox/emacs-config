...
*** The =osama-emacs-dev.el= section for Web Development
...
;; HTML Mode & Keybindings
(use-package mhtml-mode
  :ensure nil
  :bind (:map mhtml-mode-map
              ("C-c m c" . sgml-close-tag)
              ;; أضفت لك اختصارات السيرفر هنا لتكون تحت نفس البادئة
              ("C-c m s" . httpd-start)     
              ("C-c m i" . impatient-mode)  
              ("C-c m o" . (lambda () (interactive) (browse-url "http://localhost:8080/imp/")))))
