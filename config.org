...
(use-package treemacs
  ...
  :hook
  ;; (طلب جديد) إخفاء أرقام الأسطر عند فتح Treemacs
  (treemacs-mode . (lambda () (display-line-numbers-mode -1)))

  :config
  (setq treemacs-width 40)

  ;; إضافة مجلدك الخاص تلقائياً
  (when (and (boundp 'osama-emacs-home-dir)
             (file-directory-p osama-emacs-home-dir))
    (treemacs-add-project-to-workspace osama-emacs-home-dir))
...
;; Arabic Font
;; 1. إعداد الخط العربي (النوع والوزن فقط)
(when (member "Cairo" (font-family-list))
  (set-fontset-font t 'arabic
                    (font-spec :family "Cairo"
                               :weight 'bold   ; نحدد الوزن هنا
                               :slant 'normal)))

;; 2. ضبط نسبة الحجم (Scaling)
;; هذا السطر سيجعل Cairo أكبر بنسبة 1.2 من الخط الإنجليزي تلقائياً
(add-to-list 'face-font-rescale-alist '("Cairo" . 1.2))
