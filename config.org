...
* Fix Keyboard Layout
  ...
  -  Enable "Let me use a different input method for each app window" 
        Windows 11 often changes the keyboard layout when switching focus to Emacs 
        if you have multiple languages installed.
  ...
  -  Override default input method 
        - Select "Use language list (recommended)" option  
...
(use-package dired
  ...
  (:map dired-mode-map
   ...
   ;; navigation / view
   ("C-c d r" . revert-buffer)
   ("C-c d f" . dired-find-file-other-window)

   ;; file operations
   ("C-c d c" . dired-do-copy)
   ("C-c d m" . dired-do-rename)   ;; move / rename
   ("C-c d D" . dired-do-delete)
   ("C-c d n" . dired-create-empty-file)
   ("C-c d k" . dired-create-directory)

   ;; marking
   ("C-c d a" . dired-mark-files-regexp)
   ("C-c d u" . dired-unmark)
   ("C-c d U" . dired-unmark-all-marks)
   ("C-c d t" . dired-toggle-marks)

   ;; execute
   ("C-c d x" . dired-do-flagged-delete)

   ;; permissions / misc
   ("C-c d p" . dired-do-chmod)
   ("C-c d g" . dired-do-chgrp)
   ("C-c d z" . dired-do-compress)
...
*** The =osama-emacs-core.el= section for stopping mouse wheel button

#+begin_src emacs-lisp :tangle "~/.emacs.d/modules/osama-emacs-core.el"
(use-package emacs
  :init
  (dolist (key '("<mouse-2>" "<down-mouse-2>" "<drag-mouse-2>"))
    (global-unset-key (kbd key))))
#+end_src


*** The =osama-emacs-core.el= section for smoother scrolling
...
*** The =osama-emacs-ui.el= section for ~doom-modeline~ [Disabled]
...
*** The =osama-emacs-ui.el= section for ~moody~ modeline and ~minions~
...
(use-package moody
  ...
  ;; إجبار ظهور خانة الـ input method في الـ modeline
  (setcar (nth 1 mode-line-mule-info) t)

  (defun osama-update-input-method-indicator (&rest _)
    (setq current-input-method-title
          (cond
           ;; إذا كان الإدخال عربي
           ((and current-input-method
                 (string-match-p "arabic" current-input-method))
            " AR ")
           ;; أي حالة أخرى (إنجليزي / nil)
           (t
            " EN "))))

  ;; بعد أي تغيير لطريقة الإدخال
  (advice-add 'toggle-input-method :after #'osama-update-input-method-indicator)
  (advice-add 'set-input-method    :after #'osama-update-input-method-indicator)

  ;; لضمان التحديث دائمًا
  (add-hook 'post-command-hook #'osama-update-input-method-indicator)

  (setq-default mode-line-format
                '("%e"
                  mode-line-front-space
                  ;; إظهار وضع Evil كنص عادي بدون Face مسبب للمشاكل
                  (:eval
...
*** The =osama-emacs-ui.el= section for hiding ~modeline~ border


#+begin_src emacs-lisp :tangle "~/.emacs.d/modules/osama-emacs-ui.el"
(use-package emacs
  :init
  (defun osama-fix-modeline-box (&rest _)
    (set-face-attribute 'mode-line nil :box nil)
    (set-face-attribute 'mode-line-inactive nil :box nil))

  ;; بعد load-theme
  (advice-add 'load-theme :after #'osama-fix-modeline-box)

  ;; بعد enable-theme (consult-themes وغيره)
  (add-hook 'enable-theme-functions #'osama-fix-modeline-box))
#+end_src


*** The =osama-emacs-ui.el= section for ~nyan-mode~
...
(use-package evil
  ...
  ;; Global and state-specific keybindings
  :bind
...
*** The =osama-emacs-editing.el= section for =evil-mode= stoping arrow keys [Temporary]

#+begin_src emacs-lisp :tangle "~/.emacs.d/modules/osama-emacs-editing.el"
(use-package evil
  :config
  (defvar osama-disable-arrows-enabled t
    "Toggle state for disabling arrow keys.")

  (defun osama--disable-arrows ()
  ...
  ;; شغل التعطيل افتراضيًا عند Init
  (osama--disable-arrows)

  (defun osama-toggle-arrow-keys ()
    "Toggle arrow keys on/off in Evil mode."
    (interactive)
    (if osama-disable-arrows-enabled
        (progn
          (osama--enable-arrows)
          (setq osama-disable-arrows-enabled nil)
          (message "Arrow keys ENABLED"))
      (osama--disable-arrows)
      (setq osama-disable-arrows-enabled t)
      (message "Arrow keys DISABLED")))

  :bind
  ("C-c t a" . osama-toggle-arrow-keys))
#+end_src
...
*** The =osama-emacs-completion.el= section for ~corfu~, ~kind-icons~
...
*** The =osama-emacs-completion.el= section for ~cape~ 
...
(use-package cape
  ...
  :hook (eglot-managed-mode . osama-eglot-capf))
#+end_src

*** The =osama-emacs-completion.el= section for ~cape~ html (web-mode) and css smart completion 

#+begin_src emacs-lisp :tangle "~/.emacs.d/modules/osama-emacs-completion.el"
(use-package cape
  :config
  ;; 1. دالة البحث في ملفات web-mode (كما هي)
  (defun my-css-get-filtered-tokens (type)
    (let ((tokens '())
          (regexp (if (eq type ?.) "class=[\"']\\([^\"']+\\)[\"']" "id=[\"']\\([^\"']+\\)[\"']")))
      (save-excursion
              ...
              (save-excursion
                ...
                (while (re-search-forward regexp nil t)
                  (let ((match (match-string-no-properties 1)))
                    (dolist (item (split-string match " "))
                      (unless (string-empty-p item) (push item tokens))))))))))
      (delete-dups tokens)))

  ;; 2. الدالة الذكية (تم تعديلها للإظهار الفوري بمجرد كتابة الرمز)
  (defun my-css-smart-capf ()
    (when (and (derived-mode-p 'css-mode 'scss-mode) (not (bobp)))
      (let* ((end (point))
             ;; نبحث عن الرمز مباشرة خلف المؤشر أو خلف الكلمة الحالية
             (beg (save-excursion 
                    (skip-chars-backward "A-Za-z0-9_-") 
                    (point)))
             (char-before (char-before beg)))
        (when (member char-before '(?\. ?#))
          (list beg end (my-css-get-filtered-tokens char-before)
                :exclusive 'no
                ;; جعل القائمة تظهر فوراً حتى لو لم نكتب أي حرف بعد الرمز
                :company-prefix-length t 
                :company-kind (lambda (_) (if (eq char-before ?#) 'variable 'value))
                :annotation-function (lambda (_) (if (eq char-before ?#) " [ID]" " [Class]")))))))

  ;; 3. حقن الدالة (كما هي)
  (advice-add 'osama-eglot-capf :after
              (lambda ()
                (when (derived-mode-p 'css-mode 'scss-mode)
                  (setq-local completion-at-point-functions
                              (cons #'my-css-smart-capf completion-at-point-functions))))))
#+end_src
...
(use-package eglot-booster
  :if (not (eq system-type 'android)) ;; لن يتم تحميلها نهائياً في الموبايل
...
(use-package transient
  :bind ("C-c c w" . osama-web-dispatch)
  :config
    ...
    [:description "Web Server"
     ...
     ("p" "Port Settings"
      (lambda ()
        ...
        (message "⚙️ Adjusting httpd-port"))
      :transient t)]
    ...
    [:description "Live Preview"
     ("i" "Impatient Mode"
      (lambda ()
        (interactive)
        (impatient-mode)
        (message "👀 Impatient Mode toggled"))
      :transient t)
     ("o" "Open Browser"
      (lambda ()
        (interactive)
...
* TODO New Edits
- [ ] good syntex use package config https://github.com/xenodium/dotsies/blob/main/emacs/features/fe-ui.el
...
- [X] yank pop ring save file in emacs home folder like clipboard
- [X] wrap every confing with use-package macro 
- [X] make your folders clean from emacs backups (https://www.youtube.com/watch?v=XZjyJG-sFZI)
