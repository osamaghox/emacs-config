...
(use-package cape
  ...
  :demand t
  :init
  ;; 1. الإعداد العالمي (Global) - ليعمل cape في كل مكان
  (setq-default completion-at-point-functions
                (append (default-value 'completion-at-point-functions)
                        (list #'cape-dabbrev #'cape-file #'cape-abbrev)))

  ;; 2. دالة جمع الكلمات يدوياً من الـ Buffers
  (defun my-css-get-all-tokens ()
    (let ((tokens '()))
      (save-excursion
        (dolist (buf (buffer-list))
          (with-current-buffer buf
            ;; لا نبحث في ملفات CSS لتجنب التكرار
  ...
  ;; 3. الدالة الذكية لـ CSS (التي تستخدم البيانات المجمعة)
