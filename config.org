...
(use-package emacs
  ...
  (defvar osama-keyboard-toggle-state nil
    "Non-nil means Android keyboard is considered open.")

  :config
  (defun osama-toggle-keyboard ()
    "Toggle Android keyboard using isearch as a trigger."
    (interactive)
    (if osama-keyboard-toggle-state
        ;; OFF
        (progn
      ...
      (progn
        (isearch-forward)
        ;; نتركه مفتوح لحظة حتى يطلع الكيبورد
        (run-at-time
         0.4 nil
         (lambda ()
           (when (and (boundp 'isearch-mode) isearch-mode)
...
(use-package transient
  ...
  (transient-define-prefix osama-web-dispatch ()
    ...
    ["Web Server"
     ("s" "Start Server (choose directory)"
      (lambda ()
        (interactive)
        ;; اختر المجلد الذي تريد عرضه
        (let ((dir (read-directory-name "Select directory to serve: ")))
          (setq httpd-root dir)
          (httpd-serve-directory dir)
          (message "🚀 Serving %s at http://localhost:%d" dir httpd-port))) :transient t)
