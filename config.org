...
- [ ] change every keybind map to the new use-package version like this:
...
(use-package cape
  :ensure t
  :hook
  (eglot-managed-mode . my-eglot-mode-setup)
  :init
  (defun my-eglot-mode-setup ()
    ;; دمج eglot مع مصادر إكمال أخرى
    (add-hook 'completion-at-point-functions
              (cape-super-capf
               #'eglot-completion-at-point
               #'cape-dabbrev-code
               ;; #'cape-yasnippet  ;; فعّلها إذا تستخدم yasnippet
               )
              nil 'local)

    ;; حل مشاكل الكاش مع بعض LSP servers
    (advice-add 'eglot-completion-at-point
                :around #'cape-capf-buster)))
...
(use-package eglot
  :hook ((c-mode c++-mode objc-mode
          css-mode mhtml-mode js-mode web-mode) . eglot-ensure)
...
(use-package c++-mode
  ...
  :after eglot
  :hook
  (c++-mode . eglot-ensure)  ;; يشغل eldoc مع أي buffer تحت Eglot
  :bind (:map c++-mode-map
  ("C-c m v" . c-version)))
#+end_src
...
(use-package emmet-mode
  :ensure t
  :hook (web-mode mhtml-mode css-mode js-mode typescript-mode)
  :config
  (setq emmet-expand-at-point t)
  :bind (:map emmet-mode-keymap
              ("C-c m e" . emmet-expand-line)
              ;; أمثلة لأوامر إضافية مستقبلية
