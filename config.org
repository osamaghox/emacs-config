...
*** The =osama-emacs-core.el= Section for its ~global-keys~
...
(use-package emacs
  ...
  (;; Files
   ...
   ("C-c w D" . kill-buffer-and-window)
...
(use-package emacs
  :ensure nil
  :bind
   (("C-c e q" . osama-kill-emacs)
   ("C-c e c" . osama-open-emacs-config))
  :config
  ;; Function to open your config file
  (defun osama-open-emacs-config ()
...
*** The =osama-emacs-core.el= section for Windows
...
*** The =osama-emacs-core.el= section for ~tab-bar-mode~
...
*** The =osama-emacs-core.el= settings for ~dashboard~
