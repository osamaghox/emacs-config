...
(use-package cc-mode
  ...
  :bind (:map cc-mode-map
              ("C-c m c" . osama-ccc-run-and-compile))
  :config
  (defun osama-cc-run-and-compile ()
    "Compile C++, clear eshell, and prepare run command."
    (interactive)
    (let* ((src  (buffer-file-name))
      ...
      (let ((eshell-buffer-name "*cpp-eshell*"))
        (eshell)
        ;; 1. إنهاء أي عملية شغالة بقوة
        (let ((proc (get-buffer-process (current-buffer))))
          (when proc (kill-process proc)))
        
        ;; 2. مسح الشاشة بالكامل
        (let ((inhibit-read-only t))
          (erase-buffer))
        
        ;; 3. إعادة تهيئة Eshell لضمان ظهور الـ Prompt في البداية
        (eshell-mode) 
        
        ;; 4. كتابة اسم الملف البرمجي (بدون الحاجة لـ send-input فارغ)
        (goto-char (point-max))
        (insert
