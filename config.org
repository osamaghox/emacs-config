...
(use-package cape
  :ensure t
  :bind
  (("C-c c f" . cape-file)      ;; إكمال الملفات
   ("C-c c d" . cape-dabbrev)  ;; كلمات من البفرات
   ("C-c c k" . cape-keyword)  ;; كلمات اللغة
   ("C-c c s" . cape-symbol))  ;; symbols
  :init
...
(use-package lsp-mode
  :ensure t
  :diminish "LSP"
  :hook ((lsp-mode . lsp-diagnostics-mode) ; optional: displays errors/warnings
         (lsp-mode . eldoc-mode)
         (lsp-mode . lsp-enable-which-key-integration) ; optional: integration with which-key
         (web-mode . lsp-deferred)) ; Hook lsp-mode into web-mode
  :custom
  (lsp-keymap-prefix "C-c l") ; Set a prefix for LSP commands
  (lsp-completion-provider :none) ; Use another provider like 'corfu' for completion UI
