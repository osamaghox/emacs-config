...
** The =init.el= settings for system custom settings

#+begin_src emacs-lisp :tangle "~/.emacs.d/init.el" :mkdirp yes
(add-to-list 'load-path osama-emacs-dot-dir)
...
*** The =osama-emacs-appearance.el= section for ~doom-themes~
...
#+begin_src emacs-lisp :tangle "~/.emacs.d/modules/osama-emacs-fonts.el" :mkdirp yes
...
(dolist (sub '("savehist" "recentf" "desktop" "backups" "autosaves"))
  (let ((dir (expand-file-name (concat "cache/" sub "/") osama-emacs-dot-dir)))
    (unless (file-directory-p dir)
      (make-directory dir t))))

;; Savehist: command history
(setq savehist-file (expand-file-name "cache/savehist/savehist" osama-emacs-dot-dir))
(savehist-mode 1)

;; Recentf: recent files list
(setq recentf-save-file (expand-file-name "cache/recentf/recentf" osama-emacs-dot-dir))
(recentf-mode 1)

;; Desktop: full session restore
(setq desktop-path    (list (expand-file-name "cache/desktop/" osama-emacs-dot-dir))
      desktop-dirname (expand-file-name "cache/desktop/" osama-emacs-dot-dir)
      desktop-base-file-name "emacs-desktop")
(desktop-save-mode 1)

;; Backups: store ~ files in backups/
(setq backup-directory-alist
      `((".*" . ,(expand-file-name "cache/backups/" osama-emacs-dot-dir)))
      backup-by-copying t
      version-control t
...
(setq auto-save-file-name-transforms
      `((".*" ,(expand-file-name "cache/autosaves/" osama-emacs-dot-dir) t))
      auto-save-default t
...
(defvar osama-emacs-keyboard-toggle-state nil
  "Current toggle state of the Android keyboard using isearch.")

(defun osama-emacs-toggle-keyboard ()
  "Toggle the Android keyboard using isearch-forward.
If the keyboard is open, close it. If closed, open it."
  (interactive)
  (if osama-emacs-keyboard-toggle-state
      (progn
        (when (and (boundp 'isearch-mode) isearch-mode)
          (isearch-abort))  ;; Close isearch and hide the keyboard
        (setq osama-emacs-keyboard-toggle-state nil))
    (isearch-forward nil 1)  ;; Open isearch (shows the keyboard)
    (setq osama-emacs-keyboard-toggle-state t)))

;; Bind volume buttons to toggle the keyboard
(global-set-key (kbd "<volume-up>") #'osama-emacs-toggle-keyboard)
(global-set-key (kbd "<volume-down>") #'osama-emacs-toggle-keyboard)
#+end_src
