...
هذا الي جابه الاصل الي من Reddit
;;; orgdoc-mouse.el --- Display Org link content on mouse hover -*- lexical-binding: t; -*-
...
;;; Commentary:
;; Show the content of Org links in a posframe when hovering the mouse.
;; Similar to eldoc-mouse, but works on Org buffers with file/id links.
     ...
     (save-excursion
       ...
       (if (org-at-heading-p)
           (progn
             ...
             (prog1 (buffer-string)
               (widen)))
         ;; fallback: mostrar todo el buffer si no es heading
         (buffer-string))))))
  ...
  (when (and pos (number-or-marker-p pos))
    (save-excursion
      ...
      (when-let* ((ctx (org-element-context))
                  ...
                  (begin (org-element-property :begin ctx)))
        (when (eq type 'link)
          (unless (equal begin orgdoc-mouse-last-bounds)
            (setq orgdoc-mouse-last-bounds begin)
            (let* ((content (orgdoc-mouse--extract-content ctx))
                   (border-color (face-foreground 'default)))
              (when orgdoc-mouse-overlay
                (delete-overlay orgdoc-mouse-overlay))
                    ...
                    (progn
                      (posframe-show
                       ...
                       (let ((map (make-sparse-keymap)))
                         (define-key map (kbd "q") #'posframe-hide)
                         (define-key map (kbd "<wheel-up>") #'scroll-down-command)
                         (define-key map (kbd "<wheel-down>") #'scroll-up-command)
                         (define-key map (kbd "RET")
                           (lambda ()
